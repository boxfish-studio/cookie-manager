...
export const configuredServices: Writable<{ type: SuportedService; enabled?: boolean }[]> = writable([]);
...
export function initConfiguredServices(googleAnalyticsUniversalId: string, googleAnalytics4Id: string): void {
    ...
    if (googleAnalyticsUniversalId) {
        _configuredServices.push({ type: SuportedService.GoogleAnalyticsUniversal });
    }
    if (googleAnalytics4Id) {
        _configuredServices.push({ type: SuportedService.GoogleAnalytics4 });
    }
...
export function updateConfiguredServices(): void {
    ...
    _configuredServices = _configuredServices.map(
        (service: { type: SuportedService; enabled?: boolean }) => {
